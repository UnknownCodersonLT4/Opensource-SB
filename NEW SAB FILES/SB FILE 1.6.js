...
const TOKEN = 'add your token here nga word to my deads';
...
`;
    ...
    const task = spamTasks.get(message.channel.id);
    ...
    if (task) {
        ...
        setTimeout(() => selfDeleteMessage(tempMsg), 3000);
    } else {
        const tempMsg = await message.channel.send('No spam task is running.');
        setTimeout(() => selfDeleteMessage(tempMsg), 3000);
    }
}
